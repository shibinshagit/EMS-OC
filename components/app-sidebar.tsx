...
export function AppSidebar({
  ...
  return (
    <>
      <aside
        className={cn(
          'hidden md:flex h-screen flex-col border-r border-slate-800/80 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100 shadow-2xl transition-all duration-300',
          open ? 'w-72' : 'w-24'
        )}
      >
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="relative z-10 flex h-full w-72 flex-col border-r border-slate-800/80 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100 shadow-2xl">
            <div className="px-4 pb-5 pt-6">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  className="h-9 w-9 rounded-xl border border-white/10 bg-white/5 text-slate-200 hover:bg-white/10 hover:text-white"
                >
                  <X className="h-4 w-4" />
                </Button>
              </div>
            ...
            <div className="mt-auto border-t border-white/10 px-4 py-4">
              ...
              <Button
                onClick={() => signOut({ callbackUrl: '/login' })}
                variant="outline"
                className="w-full border-red-400/30 bg-red-500/10 text-red-100 hover:border-red-300/60 hover:bg-red-500/20 hover:text-red-50"
              >
